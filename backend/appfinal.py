...
def correct_text_tilt(binary_image):
    # Find contours in the binary image
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Check if contours are detected
    if len(contours) == 0:
        return binary_image  # No contours found, return the original image

    # Find the minimum area bounding box around the largest contour
    largest_contour = max(contours, key=cv2.contourArea)
    rect = cv2.minAreaRect(largest_contour)
    
    # Extract the angle of the bounding box
    angle = rect[-1]
    ...
    # If there's no significant skew, return the original image
    if abs(angle) < 0.5:  # You can adjust this threshold
        return binary_image

    # Otherwise, correct the tilt
    (h, w) = binary_image.shape[:2]
    center = (w // 2, h // 2)
    
    # Create a rotation matrix for the skew angle
...
def process_and_ocr(plate_region):
    ...
    #corrected_image = correct_text_tilt(morph_image)
