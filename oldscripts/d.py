...
def deskew_image(image):
    ...
    # Adjust the angle to ensure correct rotation
    if angle < -45:
        angle = 90 + angle
...
def resize_image(image, target_width, target_height):
    # Resize the image to the specified dimensions using cubic interpolation
...
def process_and_ocr(plate_region):
    ...
    #plate_resized = cv2.resize(plate_region_gray, (target_width, target_height), interpolation=cv2.INTER_CUBIC)
    plate_resized = resize_image(plate_region_gray, target_width, target_height)
    ...
    morph_image = cv2.morphologyEx(inverted_image, cv2.MORPH_OPEN, kernel)

# Apply morphological closing to ensure characters are properly connected
    ...
    for contour in contours:
        ...
        if area < 50:  # Adjust the threshold value depending on your needs
            cv2.drawContours(morph_image, [contour], -1, (255,), thickness=cv2.FILLED)  # Use a tuple for the color
